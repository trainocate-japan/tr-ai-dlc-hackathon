...
| 項目 | 内容 |
|------|------|
| **責務** | 閲覧/いいね済みコースのグラフデータ構築、カテゴリ関係のエッジ生成 |
| **パターン** | view-history + collections + course-summariesテーブルを参照し、ノード（コース）とエッジ（同カテゴリ関係）のグラフ構造を構築。いいね済みノードにフラグ付与。各ノードに概要サマリーを含める。 |
